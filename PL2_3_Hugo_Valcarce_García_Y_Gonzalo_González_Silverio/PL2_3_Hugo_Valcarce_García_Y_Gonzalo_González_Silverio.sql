...
SELECT 
    Dir.Nombre
FROM    
    pl2_Fin.Directores Dir INNER JOIN pl2_Fin.Personal Per ON Dir.Nombre=Per.Nombre
WHERE
    Per.Anno_Nacimiento=1970;
--Ejercicio 2
SELECT
    Idioma, COUNT(DISTINCT Titulo) AS Num_Peliculas
FROM 
    pl2_Fin.Pelicula
GROUP BY
    Idioma
...
--Ejercicio 3
SELECT 
    Act.Nombre, Per.Anno_Nacimiento
FROM 
    pl2_Fin.Actores Act INNER JOIN pl2_Fin.Personal Per ON Act.Nombre=Per.Nombre
WHERE
    Per.Anno_Nacimiento IN (SELECT
                                MAX(Anno_Nacimiento)
                            FROM
                                pl2_Fin.Personal);
--Ejercicio 4
SELECT
    Act.Nombre, COUNT(DISTINCT Pel.Titulo) AS Num_Pelicula
FROM pl2_Fin.Actores Act INNER JOIN pl2_Fin.Actuan Actu ON Act.Nombre=Actu.Actor_Nombre INNER JOIN pl2_Fin.Pelicula Pel ON Actu.Pelicula_Titulo=Pel.Titulo and Pel.Anno = Actu.Pelicula_Anno
GROUP BY
    Act.Nombre
HAVING
    COUNT(DISTINCT Pel.Titulo)>1;

--Ejercicio5
SELECT
    Gen.Genero, COUNT(DISTINCT Pel.Titulo) AS Num_Peliculas
FROM 
    pl2_Fin.Generos Gen INNER JOIN pl2_Fin.Pelicula Pel ON Gen.Pelicula_Titulo=Pel.Titulo
GROUP BY 
    Gen.Genero
ORDER BY
    Num_Peliculas DESC;
...
SELECT
    Gui.Nombre
FROM
    pl2_Fin.Guionistas Gui INNER JOIN pl2_fin.Guionizan G on Gui.Nombre = G.Guionista_Nombre INNER JOIN pl2_fin.Generos Gen on G.Pelicula_Titulo = Gen.Pelicula_Titulo
WHERE
    Gen.Genero ='Sport' OR Gen.Genero ='Film-Noir';

--Ejercicio 7

SELECT
    A.Nombre, A2.Papel, P.Titulo, P.Anno, P.Idioma, P.Duracion
FROM
    pl2_Fin.Personal Per INNER JOIN pl2_fin.Actores A on Per.Nombre = A.Nombre INNER JOIN pl2_fin.Actuan A2 on A.Nombre = A2.Actor_Nombre INNER JOIN pl2_fin.Pelicula P on P.Titulo = A2.Pelicula_Titulo and P.Anno = A2.Pelicula_Anno
WHERE
    P.Idioma= 'ja' AND P.Duracion<100 AND Per.Anno_Nacimiento<1960;
--Ejercicio 8

SELECT
    Pel.Titulo, Pel.Anno, Pel.Idioma, Pel.Duracion, Pel.Calificacion_MPA, COUNT (DISTINCT C.Nombre) AS Num_Cara
FROM
    pl2_Fin.Pelicula Pel INNER JOIN pl2_fin.Caratulas C on Pel.Titulo = C.Pelicula_Titulo and Pel.Anno = C.Pelicula_Anno
GROUP BY
    Pel.Titulo, Pel.Anno
HAVING
    COUNT (DISTINCT C.Nombre) >= 2;

--Ejercicio 9

SELECT
    Pel.Titulo, Pel.Anno, AVG(DISTINCT C.Puntuacion) AS Punt_Media
FROM
    pl2_Fin.Pelicula Pel INNER JOIN pl2_fin.Criticas C on Pel.Titulo = C.Pelicula_Titulo AND Pel.Anno = C.Pelicula_Anno
GROUP BY
    Pel.Titulo, Pel.Anno
ORDER BY
    AVG(DISTINCT C.Puntuacion) DESC
LIMIT 3;
...
WITH Punt_Media AS (
    SELECT
        G.Genero, AVG(DISTINCT C.Puntuacion) AS Pel_Media
    FROM
        pl2_Fin.Pelicula Pel INNER JOIN pl2_fin.Criticas C on Pel.Titulo = C.Pelicula_Titulo and Pel.Anno = C.Pelicula_Anno INNER JOIN pl2_fin.Generos G on Pel.Titulo = G.Pelicula_Titulo and Pel.Anno = G.Pelicula_Anno
    GROUP BY
        G.Genero
    )
SELECT
...
GROUP BY
    Genero,Pel_Media
HAVING
    Pel_Media=(SELECT MIN (Pel_Media) FROM Punt_Media);
